...
public class ReservaService {
    ...
    public ReservaResponse crearReserva(ReservaRequest request) {
        ...
        return mapToResponse(
            savedReserva,
            estudiante.getNombre() + " " + estudiante.getPrimerApellido()
        );
    }
    ...
    public ReservaResponse obtenerReserva(Long id) {
        ...
        Estudiante estudiante = estudianteRepository
            .findById(reserva.getEstudianteId())
            .orElseThrow(() ->
                new NotFoundException("Estudiante no encontrado")
            );
    ...
    public List<ReservaResponse> misReservas(Authentication authentication) {
        ...
        return estudiantes
            .stream()
            .flatMap(est ->
                reservaRepository.verMisReservas(est.getEstudianteId()).stream()
            )
            .map(this::mapToResponseSimple)
            .collect(Collectors.toList());
    }

    public List<ReservaResponse> listarReservasPorEstudiante(
        Long estudianteId
    ) {
        List<Reserva> reservas = reservaRepository.findByEstudianteId(
            estudianteId
        );
        return reservas
            .stream()
    ...
    private ReservaResponse mapToResponse(
        Reserva reserva,
    ...
    private ReservaResponse mapToResponseSimple(Reserva reserva) {
        String nombreEstudiante = estudianteRepository
            .findById(reserva.getEstudianteId())
